...
class TextRenderer {
  ...
  renderText() {
    ...
    return `<text x="150" y="125" text-anchor="middle" font-size="60px" fill="${this.textColor}">${this.text}</text>
</svg>`;
  }
  ...
  isValidTextColor() {
    ...
    const regex = new RegExp(`^#(?:[0-9a-fA-F]{3}){1,2}$|\\b(?:${validColors.join("|")})\\b`);
